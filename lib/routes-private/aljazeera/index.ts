    ...
    items = await Promise.all(
        items.slice(0, limit).map((item) =>
            cache.tryGet(item.link, async () => {
                ...
                const imageHtml = content('.article-featured-image').html() || '';
                const descriptionHtml = content('div.wysiwyg').html() || '';
                item.description = (imageHtml ? `<figure>${imageHtml}</figure>` : '') + descriptionHtml;
