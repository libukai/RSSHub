                    ...
                    $$('div.photo-box').length === 0
                        ...
                        : $$('div.photo-box')
                              ...
                              .map((i) => {
                                  ...
                                  return image.prop('src') ? { src: image.prop('src') } : undefined;
                              })
                              .filter((img): img is { src: string } => img !== undefined);
                const descriptionHtml = $$('div#article-view-content-div').html() || '';
                const imagesHtml = images.map((img) => `<figure><img src="${img.src}"></figure>`).join('');
                const description = imagesHtml + descriptionHtml;
