...
const renderDescription = (data: DescriptionData): string => {
    const parts: string[] = [];

    if (data.previewVideo) {
        parts.push(`<video controls preload="metadata" poster="${data.poster || ''}"><source src="${data.previewVideo}" type="video/webm"></video>`);
    }

    if (data.thumbs) {
        for (const t of data.thumbs) {
            parts.push(`<img src="${t.src}">`);
        }
    }
