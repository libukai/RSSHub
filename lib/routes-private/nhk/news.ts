...
import type { ArticleItem } from '../linkresearcher/types';

const baseUrl = 'https://www3.nhk.or.jp';
const apiUrl = 'https://api.nhkworld.jp';

interface NhkThumbnails {
    large?: string;
    middle?: string;
    small?: string;
    min?: string;
...
const renderNewsDescription = (img: NhkThumbnails | undefined, description: string): string => {
    const parts: string[] = [];
    if (img) {
        const src = img.large || img.middle || img.small || img.min;
        if (src) {
            parts.push(`<img src="${src}" alt="${img.alt || ''}">`, '<br>');
        }
    }
    ...
    items = await Promise.all(
        items.map((item) =>
            cache.tryGet(item.link, async () => {
                ...
                item.description = renderNewsDescription(data.data.thumbnails, data.data.detail.replace('\n\n', '<br><br>'));
                delete item.id;
                return item;
            })
        )
