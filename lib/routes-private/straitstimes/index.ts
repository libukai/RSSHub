...
const renderDescription = (images: ArticleImage[], article: string): string => {
    const parts: string[] = [];
    for (const img of images) {
        if (img.url) {
            const src = img.large || img.url;
            ...
            imgTag += '>';
            parts.push('<figure>', imgTag);
            if (img.caption) {
                parts.push(`<figcaption>${img.caption}</figcaption>`);
            }
            parts.push('</figure>');
        }
    }
