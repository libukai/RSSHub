    ...
    const items = await Promise.all(
        list.map((item) =>
            cache.tryGet(item.link, async () => {
                ...
                const $paragraph = $content('.paragraph').eq(0);
                $paragraph.find('.moreArticle').remove();
                $paragraph.find('.articleKeywordGroup').remove();

                item.description = (topImage === null ? '' : topImage) + $paragraph.html();
                item.category = [
                    ...$content("meta[property='article:tag']")
                        .toArray()
                        .map((e) => $content(e).attr('content')),
